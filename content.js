...
function onKeyDown(keyDown) {
    ...
    // Don't hijack letter/number keys while the user is typing in a form
    // field. contentEditable is intentionally excluded so hover-shortcuts
    // still work while reading rich editors / Google Docs.
    let t = keyDown.target;
    if (t && (t.nodeName === 'INPUT' || t.nodeName === 'TEXTAREA' || t.nodeName === 'SELECT')) {
        return;
    }
        ...
        case 83: // 's'
            {
                ...
                skritter +=
                    '/vocab/api/add?from=pinyinpop&ref=pinyinpop&lang=zh&word=' +
...
function onMouseMove(mouseMove) {
    ...
    if (document.caretRangeFromPoint) {
        ...
        // Neither caret API available — can't locate text under the cursor.
        return;
    }
...
function showPopup(html, elem, x, y, looseWidth) {
    ...
    popup.style.maxWidth = (looseWidth ? '' : '360px');
        ...
        if (altView === 1) {
            ...
            if (y + v + pH > window.innerHeight) {
                ...
                if (t >= 0) {
                    ...
                    y = 10; // too tall for either side — pin near top of viewport
                }
            } else  {
                y += v;
            }
...
function copyToClipboard(data) {
    if (!navigator.clipboard) {
        showPopup('Copy failed (clipboard unavailable)', null, -1, -1);
        return;
    }
    navigator.clipboard.writeText(data).then(
        () => showPopup('Copied to clipboard', null, -1, -1),
        () => showPopup('Copy failed', null, -1, -1)
    );
}

function splitPinyinStr(pinyinStr) {
    let parts = pinyinStr.trim().split(/\s+/);
    if (parts.length > 1) return parts;
    return pinyinStr.trim().match(/[a-z:]+[1-5]/g) || [pinyinStr.trim()];
}

function formatDefinition(text) {
    return text.replace(/([㐀-鿿豈-﫿]*)\[([a-zA-Z0-9 ]+)\]/g, (match, chars, pinyinStr) => {
        let syllables = splitPinyinStr(pinyinStr.toLowerCase());
        let parts = [];
        for (let syl of syllables) {
            let parsed = parse(syl);
            if (parsed) {
                let t = tonify(parsed[2], parsed[4]);
                parts.push(parsed[1] + t[0] + parsed[3]);
            } else {
                parts.push(syl);
            }
        }
        return chars + '<span class="zw-inline-py">(' + parts.join(' ') + ')</span>';
    });
}

function renderClassifiers(clString, showToneColors) {
    let items = clString.split(',');
    let html = '<div class="zw-cl-row"><span class="zw-cl-label">Measure words</span>';
    for (let item of items) {
        item = item.trim();
        let m = item.match(/^([^[]+)\[([^\]]+)\]$/);
        if (!m) continue;
        let chars = m[1].trim();
        let pinyinRaw = m[2].trim();
        let displayChar = chars.includes('|') ? chars.split('|')[1] : chars;
        let parsed = parse(pinyinRaw);
        let pinyinHtml = pinyinRaw;
        let toneNum = '5';
        if (parsed) {
            let t = tonify(parsed[2], parsed[4]);
            pinyinHtml = parsed[1] + t[0] + parsed[3];
            toneNum = parsed[4];
        }
        let toneClass = showToneColors ? ' tone' + toneNum : '';
        html += '<span class="zw-cl-item">';
        html += '<span class="zw-cl-char' + toneClass + '">' + displayChar + '</span>';
        html += '<span class="zw-cl-pinyin' + toneClass + '">' + pinyinHtml + '</span>';
        html += '</span>';
    }
    html += '</div>';
    return html;
...
function makeHtml(result, showToneColors) {
    ...
    for (let i = 0; i < result.data.length; ++i) {
        ...
        if (i > 0) {
            html += '<div class="zw-entry-sep"></div>';
        }

        // Header row: hanzi + pinyin
        html += '<div class="zw-header-row">';

        // Hanzi
        hanziClass = config.fontSize === 'small' ? 'w-hanzi-small' : 'w-hanzi';
        if (config.simpTrad === 'auto') {
            ...
            if (entry[1] !== entry[2]) {
                html += '<span class="zw-trad ' + hanziClass + '">' + entry[1] + '</span>';
            }
        }

        // Pinyin
        let pinyinClass = config.fontSize === 'small' ? 'w-pinyin-small' : 'w-pinyin';
        let p = pinyinAndZhuyin(entry[3], showToneColors, pinyinClass);
        html += '<span class="zw-pinyin-group">' + p[0] + '</span>';

        html += '</div>';
        html += '<div class="zw-sep"></div>';

        // Zhuyin (optional, shown before definitions)
        if (config.zhuyin === 'yes') {
            html += '<div class="zw-def-row">' + p[2] + '</div>';
        }

        // Definitions — each sense on its own row, max 5
        let defClass = config.fontSize === 'small' ? 'w-def-small' : 'w-def';
        let translation = entry[4].replace(/\//g, ' ◆ ');
        let senses = translation.split(' ◆ ').slice(0, 5);
        for (let s of senses) {
            s = s.trim();
            if (s.startsWith('CL:')) {
                html += renderClassifiers(s.slice(3), showToneColors);
            } else {
                html += '<div class="zw-def-row"><span class="' + defClass + '">' + formatDefinition(s) + '</span></div>';
            }
        }

        // Meta section: grammar / vocab hints
        let hasMeta = false;

        if (config.grammar !== 'no' && result.grammar && result.grammar.index === i) {
            if (!hasMeta) { html += '<div class="zw-sep"></div>'; hasMeta = true; }
            html += '<div class="zw-meta-row grammar">Press "g" for grammar and usage notes.</div>';
        }

        if (config.vocab !== 'no' && result.vocab && result.vocab.index === i) {
            if (!hasMeta) { html += '<div class="zw-sep"></div>'; hasMeta = true; }
            html += '<div class="zw-meta-row vocab">Press "v" for vocabulary notes.</div>';
        }

        texts.push([entry[2], entry[1], p[1], translation, entry[3]]);
    }

    if (result.more) {
        html += '<div class="zw-meta-row">&hellip;</div>';
    }
...
function pinyinAndZhuyin(syllables, showToneColors, pinyinClass) {
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        if (!m) {
            // Syllable doesn't match expected numbered-pinyin shape (rare CEDICT
            // entries). Render it verbatim instead of crashing the whole popup.
            html += '<span class="' + pinyinClass + '">' + syllable + '</span>';
            text += syllable;
            zhuyin += '<span class="' + (config.fontSize === 'small' ? 'w-zhuyin-small' : 'w-zhuyin') + '">'
                + syllable + '</span>';
            continue;
        }
        if (showToneColors) {
...
let miniHelp = `
    <span style="font-weight: bold;">PinyinPop — Chinese-English Dictionary</span><br><br>
